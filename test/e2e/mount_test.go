...
func replaceTemplate(templateFile string, destFile string) error {
	...
	templateBytes, err := io.ReadFile(filepath.Join(pwd, templateFile))
	if err != nil {
		return err
	}
	...
	return io.WriteFile(destFile, []byte(template), 0644)
}
...
func setupTestSuite(isTokenPassed bool) {
	if !isTokenPassed {
		...
		check(io.WriteFile(secretFile, []byte(f.testSecretID), 0644))
		...
		err = io.WriteFile(fileName, yamlData, 0644)
		if err != nil {
...
func TestMountRotateSecret(t *testing.T) {
	...
	check(io.WriteFile(secretFileA, secretA, 0644))
	...
	check(io.WriteFile(secretFileB, secretB, 0644))
	check(execCmd(exec.Command(
